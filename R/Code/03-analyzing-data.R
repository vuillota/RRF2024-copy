...
summary_table <- datasummary(
  hh_size + n_child_5 + n_elder + read + sick + female_head + 
    ...
    as.factor(district) * (Mean + SD), 
  data = hh_data,
  title = "Summary Statistics by District",
  output = file.path("R/Outputs", "summary_table.csv")  # Change to CSV
)
...
balance_table <- datasummary_balance(
  ...
  output = file.path("R/Outputs", "balance_table.csv")  # Change to CSV
)
...
stargazer(
  ...
  out = file.path("R/Outputs","regression_table.tex")
)
...
ggsave(file.path("R/Outputs", "fig1.png"), width = 10, height = 6)
...
ggsave(file.path("R/Outputs", "fig2.png"), width = 10, height = 6)
...
ggsave(file.path("R/Outputs", "fig3.png"), width = 10, height = 6)
